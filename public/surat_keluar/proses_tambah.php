...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = "INSERT INTO surat_keluar 
              (nomor_surat, tanggal_surat, tanggal_keluar, penerima, perihal, ringkasan, departemen_id, lampiran)
              VALUES 
              ('$nomor_surat', '$tanggal_surat', '$tanggal_keluar', '$penerima', '$perihal', '$ringkasan', '$departemen_id', '$newFileName')";

    $result = mysqli_query($conn, $query);

// (kode insert ke database di sini...)
if ($result) {
    logAktivitas($conn, 'Tambah Surat Keluar', "Admin ID $admin_id menambahkan Surat Keluar baru: $nomor_surat - $penerima");
