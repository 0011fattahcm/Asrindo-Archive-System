...
if (isset($_GET['id'])) {
    ...
    // Ambil data draft
    $queryGet = mysqli_query($conn, "SELECT judul, deskripsi FROM draft WHERE id='$id'");
    if ($data = mysqli_fetch_assoc($queryGet)) {
        $judul = $data['judul'] . ' - ' . substr($data['deskripsi'], 0, 50);

        // Simpan ke recycle_bin
        ...
        if ($insertRecycle) {
            ...
            $delete = mysqli_query($conn, "UPDATE draft SET deleted_at='$tanggal_hapus' WHERE id='$id'");

              if ($delete) {
                // ✅ Catat log aktivitas di sini (setelah delete sukses)
                logAktivitas($conn, 'Hapus Draft', "Admin ID $admin_id menghapus draft: $judul");
                $status = "success";
                $message = "Draft berhasil dipindahkan ke Recycle Bin!";
            } else {
                $status = "error";
                $message = "Gagal menghapus draft dari tabel utama: " . mysqli_error($conn);
            }
        } else {
            $status = "error";
            $message = "Gagal memindahkan draft ke Recycle Bin: " . mysqli_error($conn);
        }
    } else {
        $status = "error";
    ...
    $status = "error";
    $message = "ID draft tidak ditemukan!";
}
?>
<!DOCTYPE html>
<html lang="id">
<head>
  ...
  <title>Status Hapus Draft</title>
