...
$where = [];
if ($search != '') {
  $where[] = "(judul LIKE '%$search%' OR deskripsi LIKE '%$search%' OR status LIKE '%$search%')";
}
...
// Hitung total data
$countQuery = "SELECT COUNT(*) AS total FROM draft $whereSQL";
$countResult = mysqli_query($conn, $countQuery);
$totalRows = mysqli_fetch_assoc($countResult)['total'];
$totalPages = ceil($totalRows / $limit);

// Ambil data
$query = "SELECT * FROM draft $whereSQL ORDER BY created_at DESC LIMIT $offset, $limit";
...
<head>
  ...
  <title>Draft Dokumen | E-Archive PT Asrindo</title>
  ...
  <div class="ml-64 flex flex-col min-h-screen transition-all duration-300">
    <main class="flex-1 p-6 md:p-8">
      ...
      <div class="flex justify-between items-center mb-8 flex-wrap gap-3">
        <div>
          <h1 class="text-2xl font-bold text-gray-800">Draft Dokumen</h1>
          <p class="text-gray-500 text-sm">Kelola seluruh draft dokumen sebelum difinalisasi</p>
        </div>
        <a href="tambah.php"
          ...
          <i data-lucide="plus-circle" class="w-5 h-5"></i> Tambah Draft
        </a>
      </div>
      ...
      <div class="flex flex-wrap justify-between items-center mb-6 gap-3">
        <form method="GET" class="flex flex-wrap items-center gap-2">
          <input type="text" name="search" placeholder="Cari draft..."
      ...
      <?php if (isset($_GET['status'])): ?>
        <?php if ($_GET['status'] == 'success'): ?>
          <div class="auto-hide flex items-center justify-between px-4 py-3 rounded-md mb-4 bg-emerald-100 border border-emerald-400 text-emerald-700">
            <p><strong>Berhasil!</strong> Draft berhasil disimpan.</p>
          ...
          <tbody class="divide-y">
            ...
            if (mysqli_num_rows($result) > 0):
              while ($row = mysqli_fetch_assoc($result)):
                ...
                $class = $statusClass[strtolower($row['status'])] ?? 'bg-gray-100 text-gray-700';
            ?>
              <tr class="hover:bg-gray-50 transition">
                <td class="py-3 px-4"><?= $no++; ?></td>
                <td class="py-3 px-4"><?= htmlspecialchars($row['judul']); ?></td>
                <td class="py-3 px-4"><?= htmlspecialchars(substr($row['deskripsi'], 0, 50)); ?>...</td>
                <td class="py-3 px-4 text-center">
                  <span class="px-3 py-1 rounded-lg font-medium <?= $class; ?>"><?= ucfirst($row['status']); ?></span>
                </td>
                ...
                <td class="py-3 px-4 text-center">
                  <div class="flex justify-center items-center gap-4">
                    ...
                    <a href="#" onclick="event.preventDefault(); showConfirm('Yakin ingin menghapus draft ini?', function(){ window.location.href='hapus.php?id=<?= $row['id']; ?>'; });"
            ...
            <?php endwhile; else: ?>
              <tr><td colspan="6" class="text-center text-gray-500 py-4">Belum ada data draft</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
      ...
      <div class="flex flex-wrap justify-between items-center mt-6 text-sm text-gray-600 gap-4">
        <div>
          Menampilkan <b><?= $offset + 1 ?></b>–<b><?= min($offset + $limit, $totalRows) ?></b> dari <b><?= $totalRows ?></b> draft
        </div>
