...
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $tabel_asal = 'surat_masuk';
    $tanggal_hapus = date('Y-m-d H:i:s');
    $admin_id = isset($_SESSION['admin_id']) ? $_SESSION['admin_id'] : 1;
    $judul = '';

    $queryGet = mysqli_query($conn, "SELECT nomor_surat, perihal FROM surat_masuk WHERE id='$id'");
    if ($data = mysqli_fetch_assoc($queryGet)) {
        ...
        if ($insertRecycle) {
           ...
           // Soft delete (update kolom deleted_at, bukan delete)
      $delete = mysqli_query($conn, "UPDATE surat_masuk SET deleted_at='$tanggal_hapus' WHERE id='$id'");


               if ($delete) {
                // ✅ Catat log aktivitas di sini (setelah delete sukses)
                logAktivitas($conn, 'Hapus Surat Masuk', "Admin ID $admin_id menghapus surat masuk: $judul");
                $status = "success";
