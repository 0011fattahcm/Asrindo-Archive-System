...
$where = [];
if ($search != '') {
  $where[] = "(nomor_surat LIKE '%$search%' OR pengirim LIKE '%$search%' OR perihal LIKE '%$search%')";
}
...
$countQuery = "SELECT COUNT(*) AS total FROM surat_masuk $whereSQL";
$countResult = mysqli_query($conn, $countQuery);
$totalRows = mysqli_fetch_assoc($countResult)['total'];
$totalPages = ceil($totalRows / $limit);

// Ambil data sesuai filter & pagination
$query = "SELECT surat_masuk.*, departemen.nama_departemen 
          FROM surat_masuk 
          LEFT JOIN departemen ON surat_masuk.departemen_id = departemen.id 
          $whereSQL 
          ORDER BY surat_masuk.tanggal_surat DESC 
          LIMIT $offset, $limit";
...
<head>
  ...
  <title>Surat Masuk | E-Archive PT Asrindo</title>
  ...
  <main class="flex-1 p-6 md:p-8">
    ...
    <div class="flex justify-between items-center mb-6 flex-wrap gap-3">
      <div>
        ...
        <p class="text-gray-500 text-sm">Kelola semua arsip surat masuk perusahaan</p>
      </div>
      <a href="tambah.php"
    ...
    <div class="w-full bg-white rounded-xl shadow-sm p-5 mb-6 border border-gray-100">
      <form action="../laporan/export_surat_masuk.php" method="get"
    ...
    <?php if (isset($_GET['status'])): ?>
      ...
      $alert = [
        ...
        'error'   => ['#FEF3C7','#F59E0B','#92400E','Gagal menambahkan surat, silakan periksa kembali form Anda.']
      ];
      ...
      <table class="w-full text-sm text-left text-gray-700 min-w-[700px]">
        ...
        <tbody class="divide-y">
          ...
          if (mysqli_num_rows($result) > 0) {
              ...
              <tr class="hover:bg-gray-50 transition">
                ...
                <td class="py-3 px-4 text-center">
                  <div class="flex justify-center items-center gap-4">
                    <a href="detail.php?id=<?= $row['id']; ?>" class="text-emerald-600 hover:text-emerald-800" title="Lihat Detail">
                      <i data-lucide="eye"></i>
                    </a>
                    <a href="edit.php?id=<?= $row['id']; ?>" class="text-blue-600 hover:text-blue-800" title="Edit Surat">
                      <i data-lucide="pencil"></i>
                    </a>
                    <a href="#" onclick="event.preventDefault(); showConfirm('Yakin ingin menghapus surat ini?', function(){ window.location.href='hapus.php?id=<?= $row['id']; ?>'; });"
                       class="text-red-600 hover:text-red-800" title="Hapus Surat">
            ...
            <tr><td colspan="7" class="py-4 text-center text-gray-500">Belum ada data surat masuk</td></tr>
          <?php } ?>
        </tbody>
      </table>
    </div>
